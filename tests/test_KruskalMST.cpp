...
void testKruskalEdgeSelection() {
    // Test that demonstrates Kruskal's edge-by-edge selection
    EdgeWeightedGraph g(5);
    g.addEdge(Edge(0, 1, 1.0));  // Lightest - should be selected first
    g.addEdge(Edge(2, 3, 2.0));  // Second lightest - should be selected second
    g.addEdge(Edge(1, 2, 3.0));  // Third lightest - connects components
    g.addEdge(Edge(3, 4, 4.0));  // Fourth lightest - extends MST
    g.addEdge(Edge(0, 4, 10.0)); // Heavy edge - should be ignored (would complete cycle)

    KruskalMST mst(g);

    auto edges = mst.edges();
    assert(edges.size() == 4);
    assert(mst.weight() == 10.0);  // 1+2+3+4
    ...
    std::cout << "testKruskalEdgeSelection passed\n";
}
...
void testMSTProperties() {
    ...
    assert(mst.weight() == 16.0);  // Known optimal
...
void testSortedEdgeProcessing() {
    // Test that edges are processed in correct order
    EdgeWeightedGraph g(4);
    g.addEdge(Edge(0, 1, 10.0));  // Should be processed last
    g.addEdge(Edge(1, 2, 1.0));   // Should be processed first
    g.addEdge(Edge(2, 3, 5.0));   // Should be processed second
    g.addEdge(Edge(0, 3, 3.0));   // Should be processed third (completes MST)
    g.addEdge(Edge(0, 2, 8.0));   // Should be ignored (would create cycle)

    KruskalMST mst(g);

    auto edges = mst.edges();
    assert(edges.size() == 3);
    assert(mst.weight() == 9.0);  // 1+5+3

    // Should not include the heaviest edges
    std::set<double> weights;
    for (const Edge& e : edges) {
        weights.insert(e.weight());
    }
    ...
    assert(weights.count(5.0) == 1);
    assert(weights.count(8.0) == 0);
    assert(weights.count(10.0) == 0);

    std::cout << "testSortedEdgeProcessing passed\n";
}

void testCycleDetection() {
    // Test that cycles are properly detected and avoided
    EdgeWeightedGraph g(4);
    g.addEdge(Edge(0, 1, 1.0));
    g.addEdge(Edge(1, 2, 2.0));
    g.addEdge(Edge(2, 3, 3.0));
    ...
    assert(mst.weight() == 6.0);  // 1+2+3, excluding the 4.0 edge

    std::cout << "testCycleDetection passed\n";
}
...
int main() {
    std::cout << "Running KruskalMST tests...\n";
