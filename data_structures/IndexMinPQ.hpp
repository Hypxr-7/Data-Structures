...
class IndexMinPQ {
    ...
    void swim(int k) {
        while (k > 1 && keys[pq[k/2]] > keys[pq[k]]) {  // Changed comparison for min heap
            exch(k/2, k);
            k /= 2;
        }
    }
    ...
    void sink(int k) {
        while (2*k <= count) {
            ...
            if (j < count && keys[pq[j]] > keys[pq[j+1]]) ++j;  // Changed comparison for min heap
            // If parent is already smaller than both children, done
            if (keys[pq[k]] <= keys[pq[j]]) break;  // Changed comparison for min heap
            exch(k, j);
            k = j;
        }
    }
...
public:
    explicit IndexMinPQ(const int maxCap)
    ...
    int minIndex() const {
        ...
        return pq[1];  // Root of heap has minimum priority
    }
    ...
    void decreaseKey(const int i, const Key& key) {
        ...
        keys[i] = key;
        swim(qp[i]);  // Only need to swim up since key decreased (higher priority)
    }
    ...
    void increaseKey(const int i, const Key& key) {
        ...
        keys[i] = key;
        sink(qp[i]);  // Only need to sink down since key increased (lower priority)
    }
