...
void testBinarySearchTree() {
    ...
    try {
        ...
        assert(std::string(e.what()) == "Symbol table underflow");
    }
    ...
    items = bst.levelOrder();
    assert((items == std::vector<int>{1, 5, 7, 8}));

    assert(bst.size(1, 8) == 4);
    assert(bst.size(0, 9) == 4);
    assert(bst.size(2, 8) == 3);
    assert(bst.size(2, 7) == 2);
    assert(bst.size(1, 1) == 1);
    assert(bst.size(2, 2) == 0);
}
